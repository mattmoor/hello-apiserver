...
type BuildLister interface {
	...
	List(selector labels.Selector) (ret []*experimental.Build, err error)
...
// List lists all Builds in the indexer.
func (s *buildLister) List(selector labels.Selector) (ret []*experimental.Build, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*experimental.Build))
	})
	return ret, err
}
...
type BuildNamespaceLister interface {
	...
	Get(name string) (*experimental.Build, error)
...
// List lists all Builds in the indexer for a given namespace.
func (s buildNamespaceLister) List(selector labels.Selector) (ret []*experimental.Build, err error) {
...
// Get retrieves the Build from the indexer for a given namespace and name.
func (s buildNamespaceLister) Get(name string) (*experimental.Build, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(experimental.Resource("build"), name)
	}
	return obj.(*experimental.Build), nil
}
